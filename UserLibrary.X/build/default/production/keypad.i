...
# 24 "keypad.c" 2
# 53 "keypad.c"
static unsigned char readColumn();
# 63 "keypad.c"
...
extern void KEYPAD_vInit(tsKEYPAD_status *spStatus) {
    ...
    spStatus->u8BuffEndIdx = (4) - 1;

    memset(spStatus->u8KeyBuffer, 0x00, (4));
...
extern unsigned char KEYPAD_bUpdateBuffer() {
    ...
    if (spKEYPAD_status->u8BuffSize >= (4)) {
    ...
    unsigned char u8Idx = (spKEYPAD_status->u8BuffEndIdx + 1) % (4);
...
extern void KEYPAD_vClearBuffer() {
    ...
    spKEYPAD_status->u8BuffEndIdx = (4) - 1;
...
extern unsigned char KEYPAD_u8Read() {
    ...
    unsigned char u8PortA = PORTA;

    unsigned char u8PortB = PORTB;





    unsigned int u16PinMap;
    unsigned char u8Col;
    unsigned char u8Row;
    for (u8Row = 0; u8Row < (4); u8Row++) {
        ...
        switch (u16PinMap & 0xFF00) {
            case (0x0000):
                PORTA = u8PortA | (unsigned char)u16PinMap;
                u8Col = readColumn();
                PORTA = u8PortA;
                break;

            case (0x0100):
                PORTB = u8PortB | (unsigned char)u16PinMap;
                u8Col = readColumn();
                PORTB = u8PortB;
                break;
# 231 "keypad.c"
    ...
    unsigned char u8KeyNo = u8Row * (4) + u8Col;
    ...
    if (spKEYPAD_status->u16KeyChkCnt == (6) ||
        spKEYPAD_status->u16KeyChkCnt == (128) ||
        spKEYPAD_status->u16KeyChkCnt == (256)) {

        return u8KeyNo;
    } else if (spKEYPAD_status->u16KeyChkCnt == (268)) {
        spKEYPAD_status->u16KeyChkCnt = (256);
...
extern unsigned char KEYPAD_u8ReadBuffer() {
    ...
    unsigned char u8Idx = spKEYPAD_status->u8BuffBeginIdx;
    unsigned char u8KeyNo = spKEYPAD_status->u8KeyBuffer[u8Idx];
    spKEYPAD_status->u8BuffBeginIdx = (u8Idx + 1) % (4);
    spKEYPAD_status->u8BuffSize--;
...
extern unsigned char KEYPAD_u8ReadFinal() {
    ...
    unsigned char u8KeyNo = spKEYPAD_status->u8KeyBuffer[spKEYPAD_status->u8BuffEndIdx];
...
static unsigned char readColumn() {
    ...
    for (u8Col = 0; u8Col < (4); u8Col++) {
        ...
        if ((u8Port & (unsigned char)u16PinMap) != 0x00) {
            return u8Col;
        }
    }
